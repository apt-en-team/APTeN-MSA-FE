<script setup>
// 방문차량 관리 컨테이너의 방문차량 탭이다.
// 통계 카드 + 필터(검색/방문일) + 테이블 + 페이지네이션 + 상세/등록 모달로 구성한다.
...
// 방문일 필터 선택지, value는 BE targetDateType으로 전달
...
// 관리자 방문차량 페이지 응답 안전 추출
const visitorPage = computed(() => {
  const data = visitorVehicleStore.adminVisitorVehicles
...
const rows = computed(() =>
  visitorPage.value.content.map((vehicle, index) => ({
    ...vehicle,
    id: vehicle.visitorVehicleId,
...
const statsItems = computed(() => {
  const stats = visitorVehicleStore.adminVisitorVehicleStats
  return [
    { label: '오늘 방문예정', value: stats?.todayScheduled ?? 0, unit: '건', desc: '오늘 방문 예정', descClass: '' },
    { label: '등록완료', value: stats?.upcoming ?? 0, unit: '건', desc: '앞으로 올 방문', descClass: 'success' },
    { label: '이번달 전체', value: stats?.monthTotal ?? 0, unit: '건', desc: '이번 달 등록 기준', descClass: '' },
  ]
})

// 상세 조회 데이터
const detail = computed(() => visitorVehicleStore.adminVisitorVehicleDetail)
...
const buildParams = () => {
  ...
  if (filter.targetDateType) params.targetDateType = filter.targetDateType
  return params
}

// 현재 필터와 페이지로 방문차량 목록 조회
const loadVisitorVehicles = async () => {
  listLoading.value = true
  listError.value = ''
  await visitorVehicleStore.fetchAdminVisitorVehicles(buildParams())
  if (visitorVehicleStore.error) listError.value = '방문차량 목록을 불러오지 못했습니다.'
  listLoading.value = false
}

// 방문차량 통계 조회
const loadStats = async () => {
  await visitorVehicleStore.fetchAdminVisitorVehicleStats()
}
...
const openDetailModal = async (row) => {
  ...
  detailError.value = ''
  visitorVehicleStore.adminVisitorVehicleDetail = null
  await visitorVehicleStore.fetchAdminVisitorVehicleDetail(row.visitorVehicleId)
  if (visitorVehicleStore.error) detailError.value = '방문차량 상세 정보를 불러오지 못했습니다.'
...
const submitCreate = async () => {
  ...
  await visitorVehicleStore.createAdminVisitorVehicle({
    householdId,
  ...
  if (visitorVehicleStore.error) {
    createError.value = '방문차량 등록에 실패했습니다.'
    return
  }
  ...
  openResultModal({
    ...
    actionLabel: '방문차량 등록',
    afterConfirm: async () => {
      await loadVisitorVehicles()
      await loadStats()
    },
  })
}

// 마운트 시 최초 조회, 세대 캐시는 목록/통계와 독립적으로 로드
onMounted(async () => {
...
<template>
  <section class="admin-page">
    ...
    <section class="admin-page__card">
      ...
      <AdminFilterBar @search="handleSearch" @reset="handleReset">
        <input
          ...
          type="text"
          class="filter-input"
          placeholder="차량번호, 방문자명, 방문목적 검색"
          @keyup.enter="handleSearch"
        />
        <select v-model="filter.targetDateType" class="filter-input" @change="handleSearch">
          <option v-for="opt in targetDateOptions" :key="opt.value" :value="opt.value">
            {{ opt.label }}
          </option>
        </select>
      ...
      <div class="admin-page__table-shell">
        <div v-if="listLoading" class="table-feedback">방문차량 목록을 불러오는 중입니다.</div>
        ...
        >
          ...
          <template #cell-visitDate="{ row }">{{ formatDate(row.visitDate) }}</template>
      ...
      <AppPagination
        ...
        :total-pages="visitorPage.totalPages"
        :total-all="visitorPage.totalElements"
    ...
    >
      ...
      <div v-else-if="detail" class="detail-grid">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ detail.visitorName || '-' }}</span>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ formatDate(detail.visitDate) }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">방문목적</span>
          <span class="detail-value">{{ detail.visitPurpose || '-' }}</span>
        </div>
        <div class="detail-row">
      ...
      <div v-else class="table-feedback error">{{ detailError || '방문차량 정보를 찾을 수 없습니다.' }}</div>
    ...
    <BaseModal
      ...
      title="방문차량 등록"
      subtitle="세대를 선택하고 방문 예정 차량 정보를 입력합니다."
    ...
    >
      <div class="create-form">
        ...
        <div class="create-form__row">
          <label class="create-form__label">방문일</label>
          <input v-model="createForm.visitDate" type="date" class="create-form__input" />
        </div>
        <div class="create-form__row">

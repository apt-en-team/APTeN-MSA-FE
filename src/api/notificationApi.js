...
export const updateNotificationSettings = async (payload) => {
  const res = await apiClient.patch('/api/notifications/settings', payload)
  return unwrapApiData(res)
}

// FCM 토큰 등록 (/fcm-tokens 복수형)
export const registerFcmToken = async (payload) => {
  const res = await apiClient.post('/api/notifications/fcm-tokens', payload)
  return unwrapApiData(res)
}

// FCM 토큰 비활성화
export const deleteFcmToken = async (payload) => {
  const res = await apiClient.delete('/api/notifications/fcm-tokens', { data: payload })
  return unwrapApiData(res)
}

// FCM 토큰 갱신
export const updateFcmToken = async (payload) => {
  const res = await apiClient.patch('/api/notifications/fcm-tokens', payload)
  return unwrapApiData(res)
}
